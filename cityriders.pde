...
void setup()
{
  ...
  instr = loadImage("instr.png");
  icoinstr = loadImage("icoinst.png");
...
void draw()
{
  ...
  if (estado == 1) {
    ...
    image(icoinstr, width-100, 50, 35, 35);
  }
...
void paisaje()
{
  ...
  //Sol con tamaño y opacidad variable
  for (int i =0; i< 10; i++) {
...
void arboles(color cCerca, color cLejos)
{
  ...
  for (int j = 1; j <  10; j++)
  {               
    float a = random(-width/4, width/4);  //diferencia aleatoria entre las filas de arboles
    float b = random(-width/3, width/3);  //diferencia aleatoria entre las filas de arboles
    float c = random(5, 7);               //amplitud aleatoria para la segunda fila de arboles
    float d = random(20, 30);             //amplitud del ruido para las hojas
    float e = random(-width/2, width/2);  //diferencia entre los niveles de ruido

    for (int x = 0; x < width; x ++)
    {          
      float y = refy[j]; //y = reference y 
      y += 10*j*sin(3*ref_t/j + a);   //Fila de árboles más cercana (mayor amplitud, menor frecuencia).       
      y += c*j*sin(50*ref_t/j + b);   //Fila de árboles en medio (amplitud aleatoria definida antes y mayor frecuencia que el anterior)
      y += d*j*noise(5*ref_t/j +e);   //primera adición de ruido para simular las hojas (amplitud aleatoria que aumenta con la distancia, menor frecuencia).

      strokeWeight(1.5);                     //ajusta la nitidez de las hojas (más grande, hojas más suaves)
      stroke(lerpColor(cCerca, cLejos, j));  //Cambio de color en cada iteración
      line(x, y, x, height);                 //eje de oscilación de las señales sinusoidales
      ref_t += 0.02;                         //aumenta la referencia de tiempo (cambia frecuencia en la distancia)
    }
...
void mousePressed() {
  ...
  //coordenadas icono instucciones
  int xit = width-100;
  int yit = 50;
  int wit = 35;
  int hit = 35;
  ...
  if (estado != 3 && mouseX>xit && mouseX <xit+wit && mouseY>yit && mouseY <yit+hit) {
    estado = 5;
  }

  //salir de las instrucciones
  if (estado == 5 && mouseX>190 && mouseX <190+410 && mouseY>490 && mouseY <490+340) {
    estado = 1;
  }
}
